...
public class Vidlox extends AsyncTask<String, Void, Void> {
    ...
    protected Void doInBackground(String... strings) {
        ...
        try {
            /*
            url=strings[0];
            url=url.replace("/video/", "/link/")+"360/319243527a989961054d5cb794579a40";
            */
            Document doc = Jsoup.connect(strings[0])
                    .get();

            String html=doc.html().toString();
            String result=html.substring(html.indexOf("sources: [")+1, html.lastIndexOf("],"));
            String[] resultSplit=result.split("\"");

            String master=resultSplit[1];
